    ...
    while (!sawOutputEOS && !stopDecode) {
      if (!sawInputEOS) {
        ...
        if (inputBufferId >= 0) {
          ...
          if (sampleSize < 0) {
            decoder.queueInputBuffer(inputBufferId, 0, 0, 0 L, MediaCodec.BUFFER_FLAG_END_OF_STREAM);
            sawInputEOS = true;
          } else {
      ...
      if (outputBufferId >= 0) {
        ...
        if (doRender) {
          ...
          if (play_surf == null) {
            ...
            if (image != null && isImageFormatSupported(image)) {
              ...
              if (outputImageFormat != null) {
                HookMain.data_buffer = arr;
              }
              image.close();
            } else {
              XposedBridge.log("【VCAM】Image format not supported or null");
            }
          }
